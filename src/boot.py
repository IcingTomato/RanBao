...
def show_logo():
    try:
        img = image.Image("/sd/ranbao.jpg")
        set_backlight(0)
        lcd.display(img)
        for i in range(9):
            set_backlight(i)
            time.sleep(0.1)
        play_sound("/sd/cnmd.wav")

    except:
        lcd.draw_string(lcd.width()//2-100,lcd.height()//2-4, "Error: Cannot find ranbao.jpg", lcd.WHITE, lcd.RED)

    try:
        img = image.Image("/sd/use.jpg")
        set_backlight(0)
        lcd.display(img)
        for i in range(15):
            set_backlight(i)
        ...
        lcd.draw_string(lcd.width()//2-100,lcd.height()//2-4, "Error: Cannot find usage.jpg", lcd.WHITE, lcd.RED)
...
#
show_logo()
if but_b.value() == 0: #If dont want to run the demo
    set_backlight(0)
        ...
        if code_obj: # object detected
            ...
            for i in code_obj:
                ...
                if rect_size > max_rect:
                    ...
                    max_id = id
            if but_b.value() == 0:
                play_sound("/sd/voice/zh/"+str(max_id)+".wav")
